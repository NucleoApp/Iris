/*! Iris Color Picker - v1.0.7 - 2017-10-11
* https://github.com/Automattic/Iris
* Copyright (c) 2017 Matt Wiebe; Licensed GPLv2 */
(function( $, undef ){
	var _html, nonGradientIE, gradientType, vendorPrefixes, _css, Iris, UA, isIE, IEVersion;

	_html = '<div class="iris-picker"><div class="iris-picker-inner"><div class="iris-square"><a class="iris-square-value"><span class="iris-square-handle ui-slider-handle"></span></a><div class="iris-square-inner iris-square-horiz"></div><div class="iris-square-inner iris-square-vert"></div></div><div class="iris-slider iris-strip"><div class="iris-slider-offset"></div></div><div class="external-picker"><button class="iris-external-picker">Picker</button></div><div class="color-inputs-wrapper"><p class="js-hex hex-wrapper"><label for="">Hex</label><input type="text" class="color-input" value="#" maxlength="7"></p><p class="js-rgb rgb-wrapper"><label for="">RGB</label><input type="text" class="red" value="" maxlength="3"><input type="text" class="green" value="" maxlength="3"><input type="text" class="blue" value="" maxlength="3"></p></div><div class="iris-add-palette"></div></div><div class="iris-palette-container"></div></div>';
	...
	Iris = {
		...
		_create: function() {
			...
			self.controls = {
				...
				hexField:    self.picker.find('.hex-wrapper .color-input'),
				hexBlock:    self.picker.find('.hex-wrapper'),
				hexSwitcher: self.picker.find('.hex-wrapper label'),
				rgbBlock:    self.picker.find('.rgb-wrapper'),
				rgbSwitcher: self.picker.find('.rgb-wrapper label'),
				rgbFieldOne: self.picker.find('.rgb-wrapper input.red'),
				rgbFieldTwo: self.picker.find('.rgb-wrapper input.green'),
				rgbFieldThree: self.picker.find('.rgb-wrapper input.blue'),
				addColor:    self.picker.find('.iris-add-palette'),
				paletteContainer: self.picker.find('.iris-palette-container'),
				externalPicker: self.picker.find('.iris-external-picker')
			};

            // HEX value
            $(self.controls.hexField).val('#444444');
            rgb = self._hexToRGB('#444444');
			$(self.controls.rgbFieldOne).val(rgb.r);
		...
		_initControls: function() {
			...
			controls.externalPicker.on('click', function(){
				self._openExternalPicker();
			});
            controls.hexField.on('change', function () {
                ...
                self._color = self._color.fromCSS(val);
                self._change();
            });
            controls.rgbFieldOne.on('change', function(){
                var rgb, val = parseInt($(this).val(), 10);
                if(val < 0 && val > 255){
                    val = 0;
                }
                rgb = {
                    r: val,
                    g: parseInt(controls.rgbFieldTwo.val(),10),
                    b: parseInt(controls.rgbFieldThree.val(),10)
                };
                self._color = self._color.fromRgb(rgb);
                self._change();
            });
            controls.rgbFieldTwo.on('change', function () {
                var rgb, val = parseInt($(this).val(),10);
                if(val < 0 && val > 255){
                    val = 0;
                }
                rgb = {
                    r: parseInt(controls.rgbFieldOne.val(),10),
                    g: val,
                    b: parseInt(controls.rgbFieldThree.val(),10)
                };
                self._color = self._color.fromRgb(rgb);
            ...
            controls.rgbFieldThree.on('change', function () {
                var rgb, val = parseInt($(this).val(),10);
                if(val < 0 && val > 255){
                    val = 0;
                }
            ...
            controls.addColor.on('click.additional', function(){
                ...
                if (Array.isArray(self._palettes)) {
		...
		_change: function() {
			...
			controls.hexField.val( self.options.color);
			rgb = self._hexToRGB(self.options.color);
			if(rgb){
                controls.rgbFieldOne.val(rgb.r);
                controls.rgbFieldTwo.val(rgb.g);
                controls.rgbFieldThree.val(rgb.b);
			}
